...
export const createTeacher = async (req, res) => {
  try {
    ...
    if (profilePictureFile) {
      const uploadResponse = await cloudinary.uploader.upload(profilePictureFile.path);
    ...
    if (certificateFile) {
      const certUpload = await cloudinary.uploader.upload(certificateFile.path);
...
export const updateTeacher = async (req, res) => {
  try {
    ...
    if (profilePictureFile) {
      ...
      const uploadRes = await cloudinary.uploader.upload(profilePictureFile.path);
    ...
    if (certificateFile) {
      ...
      const certRes = await cloudinary.uploader.upload(certificateFile.path);
